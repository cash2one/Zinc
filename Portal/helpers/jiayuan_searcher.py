...
class JiayuanSearcher:
    ...
    def start(self, p=1):
        url = self.host + self.get_params() + '&p=' + str(p)
...
class JiayuanHelper:
    def __init__(self, searcher):
        ...
        self.region_dict = {11: '北京', 12: '天津', 13: '河北', 14: '山西', 15: '内蒙古', 21: '辽宁', 22: '吉林', 23: '黑龙江',
                       ...
                       81: '香港', 82: '澳门'}
        self.age_range = range(18, 61)
        self.height_range = range(140, 191)
        ...
        self.car_dict = {1: '暂未购车', 2: '已购车'}
        self.children_dict = {1: '无小孩', 2: '有小孩归自己', 3: '有小孩归对方'}
        self.job_dict = {1: '在校学生', 2: '计算机/互联网/IT', 3: '电子/半导体/仪表仪器', 4: '通信技术', 5: '销售', 6: '市场拓展',
        ...
        self.blood_dict = {1: 'A', 2: 'B', 3: 'O', 4: 'AB', 5: '其他'}
        ...
        self.stc_name_dict = {1: '地区', 2: '年龄', 3: '身高', 4: '教育', 5: '收入', 6: '婚姻', 7: '购房', 8: '购车',
                              13: '子女', 14: '工作', 18: '血型'}
        self.stc_value_dict = {1: self.region_dict, 2: self.age_range, 3: self.height_range, 4: self.education_dict, 5: self.income_dict,
                               6: self.marriage_dict, 7: self.house_dict, 8: self.car_dict, 13: self.children_dict, 14: self.job_dict,
                               18: self.blood_dict}
        self.scope_dict = {2: '年龄', 3: '身高', 4: '教育', 6: '婚姻'}
    ...
    def set_value(self, index, value1, value2=None):
        if value1 == 0:
            if index in self.searcher.conditions['stc']:
        ...
        if self.stc_type_dict[index] == 'select_one':
    ...
    def start_single(self, row):
        tmp = []
        for i in range(0, 20):
            res = self.searcher.start(i)
            tmp += self.get_data_of_json(res)
        raw = self.process_single(tmp, row)
        return self.generate_data(raw)
    ...
    def generate_data(self, count_dict):
        ...
        labels.sort()
        for key in labels:
            data.append(count_dict[str(key)])
