from django.test import TestCase
from Portal.helpers.loop_requester import LoopSpider, get_host, get_domain, get_full_url, get_parent, get_data, get_list


class LoopSpiderTestCase(TestCase):
    def setUp(self):
        pass

    def test_get_host_of_url(self):
        url1 = 'http://www.baidu.com'
        url2 = 'http://www.baidu.com/'
        url3 = 'https://www.baidu.com'
        url4 = 'https://www.baidu.com/'
        url5 = 'http://www.baidu.com/index'
        url6 = 'http://www.baidu.com/index.html'
        self.assertEqual(get_host(url1), url1)
        self.assertEqual(get_host(url2), url1)
        self.assertEqual(get_host(url3), url3)
        self.assertEqual(get_host(url4), url3)
        self.assertEqual(get_host(url5), url1)
        self.assertEqual(get_host(url6), url1)

    def test_get_domain_of_url(self):
        url0 = 'www.baidu.com'
        url1 = 'http://www.baidu.com'
        url2 = 'https://www.baidu.com'
        self.assertEqual(get_domain(url1), url0)
        self.assertEqual(get_domain(url2), url0)

    def test_get_parent_of_url(self):
        url1 = 'http://www.baidu.com'
        url2 = 'http://www.baidu.com/index'
        url3 = 'http://www.baidu.com/index/'
        url4 = 'http://www.baidu.com/index.html'
        url5 = 'http://www.baidu.com/index.html/'
        url6 = 'http://www.baidu.com/index/index.html'
        url7 = 'http://www.baidu.com/index/index.html/'
        self.assertEqual(get_parent(url2), url1)
        self.assertEqual(get_parent(url3), url1)
        self.assertEqual(get_parent(url4), url1)
        self.assertEqual(get_parent(url5), url1)
        self.assertEqual(get_parent(url6), url2)
        self.assertEqual(get_parent(url7), url2)

    def test_full_url_of_url(self):
        url1 = 'http://www.baidu.com'
        url2 = 'http://www.baidu.com/index'
        url3 = 'http://www.baidu.com/index.html'
        url4 = 'http://www.baidu.com/index/index.html'
        rel1 = 'test.html'
        rel2 = '/test.html'
        self.assertEqual(get_full_url(url1), url1)
        self.assertEqual(get_full_url(url2), url2)
        self.assertEqual(get_full_url(rel1, url3), 'http://www.baidu.com/test.html')
        self.assertEqual(get_full_url(rel2, url3), 'http://www.baidu.com/test.html')
        self.assertEqual(get_full_url(rel1, url4), 'http://www.baidu.com/index/test.html')
        self.assertEqual(get_full_url(rel2, url4), 'http://www.baidu.com/test.html')

    def test_get_data_of_url_success(self):
        url = 'https://raw.githubusercontent.com/trotyl/Static/master/fake_data'
        data, sth = get_data(url)
        data = data.decode('UTF-8')
        self.assertEqual(data, 'ABCD\n')

    def test_get_data_of_url_fail(self):
        url = 'https://raw.githubusercontent.com/trotyl/Static/master/not_exist'
        data, sth = get_data(url)
        self.assertFalse(data)

    def test_get_list_of_url_many(self):
        url = 'https://raw.githubusercontent.com/trotyl/Static/master/fake_baidu.html'
        data, sth = get_data(url)
        url_list = get_list(data)
        self.assertEqual(len(url_list), 127)

    def test_get_list_of_url_zero(self):
        url = 'https://raw.githubusercontent.com/trotyl/Static/master/empty.html'
        data, sth = get_data(url)
        url_list = get_list(data)
        self.assertEqual(url_list, [])

    def test_loop_state(self):
        spider = LoopSpider('https://raw.githubusercontent.com/trotyl/Static/master/index.html')
        counter = spider.start()
        self.assertEqual(counter, 5)
